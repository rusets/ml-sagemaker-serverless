...
resource "null_resource" "config_autogen" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      aws s3 cp /tmp/config.js "s3://$SITE_BUCKET/config.js" --content-type application/javascript --cache-control "no-cache, must-revalidate" >/dev/null
      aws cloudfront create-invalidation --distribution-id "$CF_ID" --paths "/config.js" "/index.html" >/dev/null
    EOT
  }
}
